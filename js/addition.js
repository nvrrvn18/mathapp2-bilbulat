function renderAddition(){
  ...
  app.innerHTML=lessonShell(
    ...
    'Gabungkan dua kelompok kartu. Jika tanda berbeda, bentuk pasangan nol lalu amati kartu yang tersisa.',
    `<div class="section pictorial-head">
      <p class="eyebrow">Eksplorasi Kartu Bilangan</p>
      <h2>Pilih jenis penjumlahan</h2>
      <div class="scenario-grid">${scenarios.map(s=>`<button class="scenario-btn" data-s="${s.id}"><span class="scenario-pic">${s.pic}</span>${s.label}</button>`).join('')}</div>
  ...
  function renderScenario(s){
    addLab.innerHTML=`
      ...
      <div class="operation-flow"><span class="active">1. Amati</span><span>2. Gabungkan</span><span>3. Pasangkan</span><span>4. Temukan hasil</span></div>
      <p class="instruction"><b>Seret</b> kartu dari Kelompok 1 dan Kelompok 2 ke Area Gabungan. Di HP, cukup ketuk kartunya.</p>
      ${s.id==='pn'?`<div class="concept-note"><b>Perhatikan:</b> pada penjumlahan positif dan negatif, hasil bergantung pada bilangan yang <b>lebih besar atau lebih banyak kartunya</b>. Setelah pasangan nol dibentuk, tanda hasil mengikuti kartu yang masih tersisa.</div>`:''}
      ...
      <div class="combine-drop" id="addCombineDrop">
        <b>Area Gabungan</b><small>Letakkan semua kartu di sini</small>
        ${verticalBoard(0,0,'addBoard')}
      </div>
    ...
    function moveCard(c){
      ...
      lane.appendChild(c);
      c.classList.add('card-arrive');
      setTimeout(()=>c.classList.remove('card-arrive'),350);
      moved++;
      addPairInfo.innerHTML=`Kartu digabungkan: <b>${moved}</b> dari ${total}`;
    ...
    function startResultStage(){
      document.querySelector('.operation-flow').innerHTML='<span>1. Amati</span><span class="active">2. Gabungkan</span><span>3. Pasangkan</span><span>4. Temukan hasil</span>';
      ...
      if(!need){
        addPairInfo.innerHTML='✓ Semua kartu sudah digabungkan. <b>Tidak ada pasangan nol</b> karena tandanya sama.';
        showAnswer(s,board);
        return;
      }
      if(pairingStarted)return;
      pairingStarted=true;
      document.querySelector('.operation-flow').innerHTML='<span>1. Amati</span><span>2. Gabungkan</span><span class="active">3. Pasangkan</span><span>4. Temukan hasil</span>';
      addPairInfo.innerHTML=`Sekarang bentuk <b>${need} pasangan nol</b>. Seret +1 atau −1 ke bagian Pasangan Nol, atau ketuk satu kartu positif lalu satu kartu negatif.`;
      enableZeroPairing(board,()=>{
        pairs++;
        addPairInfo.innerHTML=`Pasangan nol: <b>${pairs} dari ${need}</b>. Setiap +1 dan −1 bernilai 0.`;
        if(pairs===need)showAnswer(s,board);
      },()=>{});
    }
  }
  ...
  function showAnswer(s,board){
    ...
    const remain=live.reduce((t,c)=>t+(+c.dataset.v),0);
    const signText=remain>0?'positif':remain<0?'negatif':'nol';
    addStep.innerHTML=`<div id="addAnswer" class="discover-box"><p><b>Amati kartu yang tersisa.</b> Nilainya ${signText}. Berapa hasil penjumlahannya?</p>${choices(uniqueOpts(s.answer),s.answer)}</div>`;
    wireChoices(s.answer,()=>{
      done.add(s.id);
      addDone.textContent=`Sudah dieksplorasi: ${done.size} dari 3`;
      document.querySelector(`[data-s="${s.id}"]`).classList.add('completed');
      if(done.size===3){
        complete('addition');
        document.getElementById('nextArea').innerHTML='<button class="btn" onclick="go(\'subtraction\')">Lanjut ke Pengurangan →</button>';
      }
    });
  }

  function uniqueOpts(ans){
    let a=[ans,ans+1,ans-1,-ans].filter((v,i,x)=>x.indexOf(v)===i);
    while(a.length<4)a.push(ans+a.length+2);
    return a.slice(0,4).sort(()=>.5-Math.random());
  }
}
